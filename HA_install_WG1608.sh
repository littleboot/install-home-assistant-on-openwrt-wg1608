...
# modified by tom, install from local packages
# -added commands to the code, for better understanding
# -added additional "========" lines to echo print statements so they can be identified more easilly in the output
...
set -e # The -e option instructs a shell to exit if a command yields a non-zero exit status. Simply put, the shell exits when the command fails.

OPENWRT_VERSION=${OPENWRT_VERSION:-22.03}
HOMEASSISTANT_MAJOR_VERSION="2022.12"
export PIP_DEFAULT_TIMEOUT=100

# Gets latest full HA version number that matches with the major version. example return: "2022.12.0"
get_ha_version()
{
  wget -q -O- https://pypi.org/simple/homeassistant/ | grep ${HOMEASSISTANT_MAJOR_VERSION} | tail -n 1 | cut -d "-" -f2 | cut -d "." -f1,2,3
}

# Store HA version number
HOMEASSISTANT_VERSION=$(get_ha_version)

# Error check: Stop script if HA version does not exist
...
# Returns python version availble on openwrt package repo, example return: "3.10"
get_python_version()
{
  opkg list | grep python3-base | head -n 1 | grep -Eo '\d+\.\d+'
}

# returns package version number required by HA install, The package name needs to be passed to this function
get_version()
{
  local pkg=$1
...
# adds version number, required by HA, to the package name passed to this function
version()
{
  local pkg=$1
...
# Check if the device is a lumi_gateway by checking for a specific hardware device
is_lumi_gateway()
{
  ls -1 /dev/ttymxc1 2>/dev/null || echo ''
}

# download lists of HA requirements, and combine these into a single file:  ha_requirements.txt
...
# NabuCasa/hass-nabucasa == Home Assistant Cloud
# now we can fetch nabucasa version and its deps
# example: wget -q https://raw.githubusercontent.com/NabuCasa/hass-nabucasa/0.61.0/setup.py -O - | grep '[>=]=' | sed -E 's/\s*"(.*)",?/\1/' >> /tmp/ha_requirements.txt
# NOTE, text that is added has a tab indentation and also a string "python requires". which is not a package I think
...
PYCOGNITO_VER=2022.08.0  # Modified so it matches the requirements in the list 2022.8.0, needs additional zero in front of the 2022.08.0
HOMEASSISTANT_FRONTEND_VERSION=$(get_version home-assistant-frontend) #example: "20221207.0"
IPP_VER=$(get_version pyipp) # example: "0.12.1"
PYTHON_MIIO_VER=$(get_version python-miio) # example: "0.5.12"
AIODISCOVER_VER=$(get_version aiodiscover) # example: "1.4.13"
NABUCASA_VER=$(get_version hass-nabucasa) # example: "0.61.0"

# Check if there is a running HA process on the system, if this is the case end this process to freeup resources for the install.
...
echo "=========================================="
echo "Update opkg installable packages list"
echo "=========================================="
opkg update
...
### opkg install fixes
# fix for: python3-botocore
opkg install http://downloads.openwrt.org/snapshots/packages/mipsel_24kc/packages/python3-docutils_0.19-1_mipsel_24kc.ipk
opkg install http://downloads.openwrt.org/snapshots/packages/mipsel_24kc/packages/python3-botocore_1.12.66-2_mipsel_24kc.ipk 

# fix, later in the script I got a warning about this package missing. installing this will fix it
opkg install http://downloads.openwrt.org/snapshots/packages/mipsel_24kc/packages/python3-distutils_3.10.7-1_mipsel_24kc.ipk

# fix for: python3-gdbm (missing in stable and snapshot repo, but available in previous release)
opkg install http://downloads.openwrt.org/releases/packages-21.02/mipsel_24kc/packages/python3-gdbm_3.9.15-1_mipsel_24kc.ipk

# NOTE, numpy missing! Custom compile needed for this package

cd /tmp/

# add missing _distutils_hack from setuptools to python site-packages
...
# remove files and folders (not sure what why this is needed)
...
echo "=========================================="
echo "Install base requirements from PyPI using pip"
echo "=========================================="
...
$(version pyotp)
$(version gTTS)
$(version aioesphomeapi) #conflict with aihttp, TODO fix aiohtt dependencies
$(version zeroconf)
...
# extra services
hass-configurator==0.5.2 #changed
EOF
...
# fix deps of opkg installed packages async_timeout<4.3,>=3.0
sed -i 's/async_timeout<4.0,>=3.0/async_timeout>=3.0/' /usr/lib/python${PYTHON_VERSION}/site-packages/aiohttp-*.egg-info/requires.txt
sed -i 's/urllib3 (<1.27,>=1.25.4)/urllib3 (<1.27,>=1.25.11)/' /usr/lib/python${PYTHON_VERSION}/site-packages/botocore-*.dist-info/METADATA #fix
sed -i 's/botocore (<1.30.0,>=1.29.26)/botocore (<1.30.0,>=1.12.66)/' /usr/lib/python${PYTHON_VERSION}/site-packages/boto3-*.dist-info/METADATA #fix


# download some python packages and modify there requirements before install so they will work with the packages installed using opkg
echo "Download files"

wget https://github.com/pvizeli/pycognito/archive/refs/tags/${PYCOGNITO_VER}.tar.gz -O - > pycognito-${PYCOGNITO_VER}.tgz
wget https://pypi.python.org/packages/source/p/pyipp/pyipp-${IPP_VER}.tar.gz -O - > pyipp-${IPP_VER}.tgz.gz
...
tar -zxf pyipp-${IPP_VER}.tgz.gz
cd pyipp-${IPP_VER}
sed -i 's/aiohttp>=[0-9\.]*/aiohttp/' setup.py
sed -i 's/yarl>=[0-9\.]*/yarl/' setup.py
python3 setup.py install
cd ..
